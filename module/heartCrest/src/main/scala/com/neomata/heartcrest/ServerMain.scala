package com.neomata.heartcrest
...
object ServerMain extends App with StrictLogging {
  val host = Try(args(0)).getOrElse("192.168.0.119")
  val port = Try(args(1).toInt).getOrElse(80)
  logger.info("Hosting at {}:{}", host, port)

  implicit val system: ActorSystem[_] = ActorSystem(Basis(), "server")
  val router = new com.neomata.heartcrest.server.Router(host, port)
